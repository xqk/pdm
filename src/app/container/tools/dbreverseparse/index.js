  ...
  const onOK = (e, {updateStatus}) => {
    ...
    if (selectedTable?.length > 0) {
      const properties = (dbConn.filter(d => d.defKey === dbData.defKey)[0] || {})?.properties
        || {};
      const dbConnDefName = (dbConn.filter(d => d.defKey === dbData.defKey)[0] || {})?.defName || '';
      const tablePrefix = `${dbConnDefName}.`;
          ...
          let tempData = data.body.map((d) => {
            ...
            if (dbData.flag === 'LOWCASE') {
              ...
              return {
                ...d,
                group,
                fields: (d.fields || []).map(f => ({...f, defKey: f.defKey})),
                defKey: tablePrefix + d.defKey,
              };
            }
            return {
              ...d,
