...
export default React.memo(({
                               prefix, data, dataSource, BaseExtraCom, customerHeaders,
    ...
    const onDrop = (e) => {
        ...
        if (dragData) {
            ...
            tableRef.current.updateTableData((pre) => {
                const success = newFields.filter(f => (pre.fields || []).findIndex(eFiled => eFiled.defKey === f.defKey) < 0);
              ...
              <span>{FormatMessage.string({id: 'tableBase.other'})}</span>
                ...
                >
                  <FormatMessage id='tableBase.nameTemplate'/>
                                ...
                                <span className={`${currentPrefix}-form-item-component`}>
                  <Input
                      defaultValue={data.nameTemplate || '{defKey}[{defName}]'}
                         onChange={e => onChange(e, 'nameTemplate')}/>
                </span>
                            </div>
                            <div className={`${currentPrefix}-form-item`}>
                <span
                ...
                >
                  ...
                  >
                    {
                        dataSource && dataSource.viewGroups && (dataSource?.viewGroups).map(v => <Option key={v.id} value={v.id}>{`${v.defKey}(${v.defName || v.defKey})`}</Option>)
                    }
                  </MultipleSelect>
                </span>
                            </div>
